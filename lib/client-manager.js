...
ClientManager.prototype.addSocket = function (socket) {
  ...
  socket.on('authenticate', function(message, ackCallback) {
    thisClientManager.authenticateClient(socket, message, socket.domain, ackCallback);
  });

  socket.on('join-token-channel', function(message) {
    thisClientManager.joinTokenChannel(socket.id, message, socket.domain);
  });
 ...
 */
ClientManager.prototype.authenticateClient = function (client, message, domain, ackCallback) {
  ...
  if (this.authenticatedClients[message.authToken]) {
    ...
    this.setupClientConnection(client.id, this.authenticatedClients[message.authToken], message.contentTokens, domain);
  }
  else {
 ...
 */
ClientManager.prototype.joinTokenChannel = function (socketId, message, domain) {
  if (!this.sockets[socketId]) {
    // This socket is not authenticated yet.
    return;
  }

  var channel = `${domain}.${message.channel}`;
  ...
  if (this.tokenChannels[channel].tokens[token]) {
    ...
    this.publishMessageToContentChannel(channel, domain, notificationMessage);
  }
  else {
...
ClientManager.prototype.setupClientConnection = function (sessionId, authData, contentTokens, domain) {
  ...
  for (var i in authData.channels) {
    var channelsKey = `${domain}.${authData.channels[i]}`;
    this.channels[channelsKey] = this.channels[channelsKey] || {'sessionIds': {}};
    this.channels[channelsKey].sessionIds[sessionId] = sessionId;
  }
  if (authData.uid != 0) {
  ...
  for (var tokenChannel in contentTokens) {
    var tokenChannelsKey = `${domain}.${tokenChannel}`;
    // @TODO: Need to check contentTokens.hasOwnProperty()?
    this.tokenChannels[tokenChannelsKey] = this.tokenChannels[tokenChannelsKey] || {'tokens': {}, 'sockets': {}};

    clientToken = contentTokens[tokenChannel];
    if (this.tokenChannels[tokenChannelsKey].tokens[clientToken]) {
      this.tokenChannels[tokenChannelsKey].sockets[sessionId] = this.tokenChannels[tokenChannelsKey].tokens[clientToken];

      this.logger.debug("setupClientConnection: Added token " + clientToken + " for channel " + tokenChannel + " for socket " + sessionId);

      delete this.tokenChannels[tokenChannelsKey].tokens[clientToken];
    }
  }
    ...
    if (this.tokenChannels[tokenChannel].sockets[socket.id]) {
      ...
      if (this.tokenChannels[tokenChannel].sockets[socket.id].notifyOnDisconnect) {
        ...
        var timeoutFunction = (function (tokenChannel, uid) {
          return function () {
            thisClientManager.checkTokenChannelStatus(tokenChannel, uid, socket.domain);
          };
 ...
 */
ClientManager.prototype.getContentTokenChannelUsers = function (channel, domain) {
  var users = {uids: [], authTokens: []};

  var tokenChannelsKey = `${domain}.${channel}`;
  if (!this.tokenChannels[tokenChannelsKey]) {
    return users;
  }

  for (var sessionId in this.tokenChannels[tokenChannelsKey].sockets) {
 ...
 * Set a content token.
 */
ClientManager.prototype.setContentToken = function (channel, token, value, domain) {
  const key = `${domain}.${channel}`;
  this.tokenChannels[key] = this.tokenChannels[key] || {'tokens': {}, 'sockets': {}};
  this.tokenChannels[key].tokens[token] = value;
};
 ...
 *   True on success; false on error.
 */
ClientManager.prototype.publishMessageToContentChannel = function (channel, domain, message) {
  var key = `${domain}.${channel}`;
  if (!this.tokenChannels.hasOwnProperty(key)) {
    this.logger.log("publishMessageToContentChannel: The channel with domain " + channel + " doesn't exist.");
    return false;
  }

  for (var socketId in this.tokenChannels[key].sockets) {
 ...
 */
ClientManager.prototype.checkTokenChannelStatus = function (tokenChannel, uid, domain) {
  var tokenChannelsKey = `${domain}.${tokenChannel}`;
  ...
  for (var i = 0; i < sessionIds.length; i++) {
    if (this.tokenChannels[tokenChannelsKey].sockets[sessionIds[i]]) {
  ...
  for (var socketId in this.tokenChannels[tokenChannelsKey].sockets) {
    this.publishMessageToClient(socketId, message);
  }
};
 ...
 */
ClientManager.prototype.broadcastMessage = function (domain, message) {
  this.logger.debug("broadcastMessage", message);

  var domainChannels = this.tokenChannels[domain];

  for (var channel in domainChannels) {
    for (var socketId in channel.sockets) {
      this.publishMessageToClient(socketId, message);
    }
  }
};
