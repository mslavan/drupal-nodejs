...
Routes.prototype.getContentTokenUsers = function (request, response) {
  ...
  var users = request.clientManager.getContentTokenChannelUsers(request.body.channel, request.headers.host);
...
Routes.prototype.setContentToken = function (request, response) {
  ...
  request.clientManager.setContentToken(request.body.channel, request.body.token, request.body, request.headers.host);
...
Routes.prototype.publishMessageToContentChannel = function (request, response) {
  ...
  var result = request.clientManager.publishMessageToContentChannel(request.body.channel, request.headers.host, request.body);
  if (result) {
    response.send({status: 'success'});
  }
  else {
